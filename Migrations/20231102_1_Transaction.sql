...
  IN pAmount DECIMAL(10,2), -- Transaction amount
...
  IF NOT EXISTS (
    SELECT 1 FROM bank_account WHERE account_no = pAccountNo
  ) THEN
...
  IF pTransactionType IN (2, 3) AND (
    SELECT account_type FROM bank_account WHERE account_no = pAccountNo
  ) = 1 THEN
...
  INSERT INTO transactions (account_no, transaction_type, amount, reference, time_stamp) VALUES (
    pAccountNo, pTransactionType, pAmount, pReference, NOW()
  );
